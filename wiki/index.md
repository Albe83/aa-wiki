# Index

## Overview
- [[overview]] — Artificial Analysis overview: independent AI benchmarking platform

## Sources
- [[sources/aa-wer-v2]] — AA-WER v2.0 launch: AA-AgentTalk dataset, cleaned VoxPopuli/Earnings22 | ingested 2026-07-21
- [[sources/guide]] — AA usage guide: how to choose model and provider | ingested 2026-07-21
- [[sources/methodology]] — Official AA methodology: scope, definitions, measurement standards | ingested 2026-07-21
- [[sources/intelligence-benchmarking]] — Intelligence Index v4.1: composition and benchmarks | ingested 2026-07-21
- [[sources/performance-benchmarking]] — API performance: workloads, metrics, integrity | ingested 2026-07-21
- [[sources/capability-indices]] — Skill and Industry Indices: composition and weights | ingested 2026-07-21
- [[sources/system-load-test]] — AA-SLT: system load benchmark | ingested 2026-07-21
- [[sources/coding-agents-benchmarking]] — Coding Agent Index v1.2: 321 tasks across 3 components | ingested 2026-07-21
- [[sources/agentperf]] — AA-AgentPerf: hardware benchmark for agentic deployments | ingested 2026-07-21
- [[sources/image-methodology]] — Image generation benchmark: quality, speed, price | ingested 2026-07-21
- [[sources/music-methodology]] — Music Arena: blind pairwise voting for music | ingested 2026-07-21
- [[sources/openness-index-methodology]] — Openness Index: weight, data, methodology transparency | ingested 2026-07-21
- [[sources/speech-to-speech-methodology]] — Speech-to-Speech Index: 3 dimensions (33/33/33) | ingested 2026-07-21
- [[sources/speech-to-text-methodology]] — AA-WER v2.2: STT with streaming evaluation | ingested 2026-07-21
- [[sources/text-to-speech-methodology]] — TTS Arena: Controlled vs Provider Voices | ingested 2026-07-21
- [[sources/video-methodology]] — Video Arena: 6 modalities, hourly Elo | ingested 2026-07-21

## Entities

### Organizations
- [[entities/orgs/artificial-analysis]] — Independent AI benchmarking platform, creator of all AA benchmarks
- [[entities/orgs/aws]] — Amazon Web Services / Bedrock, cloud provider
- [[entities/orgs/openai]] — AI lab, creator of GPT, AA tokenization standard
- [[entities/orgs/meta]] — Meta AI, creator of Llama and Muse
- [[entities/orgs/together-ai]] — Inference provider for open-weight models
- [[entities/orgs/anthropic]] — AI lab, creator of Claude (Opus)
- [[entities/orgs/google]] — Google DeepMind / Gemini
- [[entities/orgs/deepseek]] — Chinese AI lab, open-weight models
- [[entities/orgs/mistral]] — French AI lab, open-weight models
- [[entities/orgs/kimi]] — Moonshot AI, creator of Kimi K3
- [[entities/orgs/spacexai]] — Elon Musk's xAI (Grok)
- [[entities/orgs/minimax]] — Chinese AI lab
- [[entities/orgs/nvidia]] — NVIDIA AI
- [[entities/orgs/alibaba]] — Alibaba Cloud / Qwen
- [[entities/orgs/xiaomi]] — Xiaomi AI
- [[entities/orgs/z-ai]] — Z AI
- [[entities/orgs/thinking-machines]] — Thinking Machines (Inkling)
- [[entities/orgs/cohere]] — Cohere AI
- [[entities/orgs/mbzuai]] — MBZUAI Institute of Foundation Models
- [[entities/orgs/tencent]] — Tencent AI (Hy)
- [[entities/orgs/sierra]] — Developer of the τ-bench benchmark (τ³-Banking)
- [[entities/orgs/stanford]] — Stanford University, developer of Terminal-Bench

## Concepts
- [[concepts/lufs-normalization]] — ITU-R BS.1770 loudness normalization for the Music Arena
- [[concepts/model-selection-framework]] — AA process: use-case → model → provider
- [[concepts/blended-price]] — 7:2:1 blended price (cache hit : input : output)
- [[concepts/cost-per-task]] — Weighted average cost per task in the Intelligence Index
- [[concepts/model-ecosystem]] — Model, Model Creator, Endpoint, Provider, System distinction
- [[concepts/open-weights]] — Public-weight vs proprietary models, AA classification
- [[concepts/output-speed]] — Performance metrics: TTFT, Output Speed, Response Time
- [[concepts/serverless]] — Pay-per-use in the LLM API context
- [[concepts/streaming-latency]] — Latency metrics for STT streaming: final and partial transcript
- [[concepts/swatlas-qna]] — SWE-Atlas-QnA: 124 repository Q&A tasks, Coding Agent Index component
- [[concepts/token]] — OpenAI Tokens vs Native Tokens, AA measurement standard
- [[concepts/intelligence-index]] — Artificial Analysis Intelligence Index: composite intelligence index
- [[concepts/gdpval-aa]] — GDPval-AA v2: benchmark on real work tasks
- [[concepts/tau3-banking]] — τ³-Banking: agentic tool use benchmark
- [[concepts/terminal-bench]] — Terminal-Bench: agentic coding benchmark
- [[concepts/aa-omniscience]] — AA-Omniscience: knowledge and hallucination benchmark
- [[concepts/aa-briefcase]] — AA-Briefcase: long-duration knowledge work benchmark
- [[concepts/aa-lcr]] — AA-LCR: long context reasoning benchmark
- [[concepts/openness-index]] — Openness Index: model openness and transparency index
- [[concepts/bradley-terry]] — Statistical model for ranking from pairwise comparisons (used in all AA arenas)
- [[concepts/voxpopuli-cleaned-aa]] — Cleaned EU parliamentary speech dataset, 628 samples, 25% AA-WER weight
- [[concepts/word-error-rate]] — WER metric: formula, normalization, Levenshtein distance
- [[concepts/cache-pricing]] — Cache pricing models: cache hit, write, storage
- [[concepts/full-duplex]] — Simultaneous bidirectional conversation in speech-to-speech models
- [[concepts/scicode]] — SciCode: scientific code generation benchmark
- [[concepts/humanitys-last-exam]] — HLE: extreme reasoning benchmark
- [[concepts/gpqa-diamond]] — GPQA Diamond: scientific reasoning benchmark
- [[concepts/critpt]] — CritPt: critical reasoning benchmark
- [[concepts/deepswe]] — DeepSWE: 113 long-horizon SE tasks, Coding Agent Index component
- [[concepts/earnings22-cleaned-aa]] — Cleaned corporate earnings calls dataset, 6 samples, 25% AA-WER weight
- [[concepts/harvey-lab-aa]] — Harvey LAB-AA: legal benchmark developed with Harvey
- [[concepts/apex-agents-aa]] — APEX-Agents-AA: agentic benchmark
- [[concepts/automationbench-aa]] — AutomationBench-AA: automation benchmark
- [[concepts/enterpriseops-gym-aa]] — EnterpriseOps-Gym-AA: enterprise IT ops benchmark
- [[concepts/itbench-aa]] — ITBench-AA: IT benchmark
- [[concepts/livecodebench]] — LiveCodeBench: live coding benchmark
- [[concepts/ifbench]] — IFBench: instruction following benchmark
- [[concepts/mmlu-pro]] — MMLU-Pro: advanced multidisciplinary benchmark
- [[concepts/global-mmlu-lite]] — Global MMLU-Lite: global multidisciplinary benchmark
- [[concepts/mmmu-pro]] — MMMU-Pro: advanced multimodal benchmark
- [[concepts/capability-indices]] — Capability indices for skills and industries
- [[concepts/coding-agent-index]] — Agentic coding composite index
- [[concepts/aa-slt]] — System Load Test: AA system load benchmark
- [[concepts/aa-agenttalk]] — Proprietary held-out STT dataset: voice agent speech, 469 samples, 50% AA-WER weight
- [[concepts/aa-agentperf]] — AgentPerf: hardware benchmark for agentic deployments
- [[concepts/image-arena]] — Qualitative evaluation arena for image generation
- [[concepts/music-arena]] — Qualitative evaluation arena for music generation
- [[concepts/speech-to-speech-index]] — S2S composite index: reasoning, conversation, agentic
- [[concepts/speech-to-text]] — STT benchmark: AA-WER Index + streaming evaluation
- [[concepts/text-to-speech-arena]] — TTS Arena: Controlled Voices vs Provider Voices
- [[concepts/video-arena]] — Qualitative evaluation arena for video generation
- [[concepts/aa-wer]] — Word Error Rate benchmark v2.2 for speech-to-text
- [[concepts/big-bench-audio]] — Audio adaptation of Big Bench Hard for speech reasoning
- [[concepts/tau-voice]] — Agentic voice benchmark with 278 customer service scenarios

## Comparisons

## Questions
